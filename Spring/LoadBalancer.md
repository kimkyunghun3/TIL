...
User -> Internet -> Web Server -> Database Server (No load balancing)
로드 밸런싱이 필요하지 않습니다.
...
User -> Load Balancer -> web-backend       ->       Database
...
Client -> Proxy1 -> Proxy2 -> HTTP-Server/Balancer -> GrizzlOX-Cluster
순으로 진행된다.
...
장애가 났을 경우 시나리오
Active했던 것이 장애가 생겼을 경우 다른 Load balancer을 active시켜서 전송시키면 된다.
